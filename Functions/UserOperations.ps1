...
function CreateNewUser {
    ...
    try {
        $script:connectionStatus = "正在检查账号是否存在[$username]..."
        UpdateStatusBar
        $script:mainForm.Refresh()
        ...
        $userExists = Invoke-Command @checkUserParams
        if ($userExists) {
            [System.Windows.Forms.MessageBox]::Show("账号[$username]已存在，请使用其他账号名", "创建失败", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            $script:connectionStatus = "创建失败：账号已存在"
            UpdateStatusBar
            return
        }
