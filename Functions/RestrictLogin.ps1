function ShowRestrictLoginForm {
    ...
    $script:allowedOriginalItems = @()  # 初始化为空数组
    ...
    $lblAllowedCount.Text = "允许登录计算机共 0 台,(0台表示允许登录所有计算机)"
    ...
    $btnPanel.Dock = "Fill"
    # 增大顶部内边距使按钮下移至中间（原30调整为50，根据窗口高度计算）
    ...
    function UpdateCounts {
        $lblAllowedCount.Text = "允许登录计算机共 $($lstAllowed.Items.Count) 台,(0台表示允许登录所有计算机)"
	...
	function FilterList($sourceList, $targetList, $originalItems, $searchText, $placeholder) {
        ...
        if (-not $originalItems) { $originalItems = @() }
		
		if ([string]::IsNullOrWhiteSpace($actualSearchText)) {
			# 搜索为空时显示所有原始项（过滤null值）
			$originalItems | Where-Object { $_ -ne $null } | ForEach-Object { $targetList.Items.Add($_) | Out-Null }
		}
		else {
			# 模糊匹配（不区分大小写，过滤null值）
			$lowerSearch = $actualSearchText.ToLower()
			$originalItems | Where-Object { $_ -ne $null -and $_.ToLower().Contains($lowerSearch) } | ForEach-Object {
    ...
    function LoadDomainComputers {
        try {
            $script:computerNameMap = @{}
            $script:allowedOriginalItems = @()  # 强制初始化为空数组
            $script:allComputersOriginalItems = @()
            ...
            $script:allComputersOriginalItems = @()
            UpdateCounts
            # $restrictForm.Close() # 可选：注释掉，让用户看到错误后手动关闭
        }
    }
    ...
    # 全部添加到允许列表
    $btnAddAll.Add_Click({
        if (-not $script:allowedOriginalItems) { $script:allowedOriginalItems = @() }
        if (-not $script:allComputersOriginalItems) { $script:allComputersOriginalItems = @() }
