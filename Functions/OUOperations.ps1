...
function SwitchOU {
    ...
    $okButton.Add_Click({
        if ($ouListBox.SelectedItem) {
            ...
            # ---------------------- 新增：清空搜索框，确保默认全显 ----------------------
            $script:textSearch.Text = ""  # 清空用户搜索框
            $script:textGroupSearch.Text = ""  # 清空组搜索框
            # --------------------------------------------------------------------------
    ...
    if ($ouForm.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
        $displayName = switch -Wildcard ($script:currentOU) {
            "^CN=Users," { "默认Users容器" }
            "^CN=Users," { "所有Users容器" }
            default { $script:currentOU.Split(',')[0] -replace 'CN=', '' }  # 优化显示格式（只显示OU名称）
        }
        $script:connectionStatus = "已切换到OU：$displayName"
        UpdateStatusBar
        
        # 切换OU后刷新用户和组列表（依赖已有分页逻辑）
        LoadUserList
        LoadGroupList
    }
}
