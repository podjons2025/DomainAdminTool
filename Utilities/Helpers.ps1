...
#用户账号搜索过滤逻辑封装
function Update-SearchUsersResults {
    $filterText = $script:textSearch.Text.ToLower()
    $script:filteredUsers.Clear()

    # 过滤逻辑
    if ([string]::IsNullOrEmpty($filterText)) {
        $script:allUsers | ForEach-Object { $script:filteredUsers.Add($_) | Out-Null }
    } else {
        $script:allUsers | Where-Object {
            ( (-not [string]::IsNullOrEmpty($_.DisplayName)) -and $_.DisplayName.ToLower() -like "*$filterText*" ) -or
            $_.SamAccountName.ToLower() -like "*$filterText*" -or
            ( (-not [string]::IsNullOrEmpty($_.EmailAddress)) -and $_.EmailAddress.ToLower() -like "*$filterText*" ) -or
            ( (-not [string]::IsNullOrEmpty($_.TelePhone)) -and $_.TelePhone.ToLower() -like "*$filterText*" ) -or
            ( (-not [string]::IsNullOrEmpty($_.Description)) -and $_.Description.ToLower() -like "*$filterText*" ) -or
            ( (-not [string]::IsNullOrEmpty($_.MemberOf)) -and $_.MemberOf.ToLower() -like "*$filterText*" )
        } | ForEach-Object { $script:filteredUsers.Add($_) | Out-Null }
    }
...
#用户账号搜索过滤逻辑封装
function Update-SearchGroupsResults {
...
function Check-ScrollBar {
    ...
    # 1. 判断是否需要横向滚动条（列总宽度 > 控件宽度）
    $totalColumnWidth = 0
    foreach ($col in $dgv.Columns) {
        # 累加所有列的宽度（包括隐藏列，因为隐藏列也会影响总宽度）
        $totalColumnWidth += $col.Width
    }
    # 控件可用宽度 = 客户端宽度 - 纵向滚动条宽度（约17px，避免计算误差）
    $dgvAvailableWidth = $dgv.ClientSize.Width - 17
    $needHorizontal = $totalColumnWidth -gt $dgvAvailableWidth

    # 2. 判断是否需要纵向滚动条（行总高度 > 控件高度）
    $totalRowHeight = $dataCount * $dgv.RowTemplate.Height + $dgv.ColumnHeadersHeight
    # 控件可用高度 = 客户端高度 - 横向滚动条高度（约17px）
    $dgvAvailableHeight = $dgv.ClientSize.Height - 17
    $needVertical = $totalRowHeight -gt $dgvAvailableHeight

    # 3. 根据判断结果设置滚动条类型
    if ($needHorizontal -and $needVertical) {
        $dgv.ScrollBars = [System.Windows.Forms.ScrollBars]::Both  # 同时显示横纵
    }
    elseif ($needHorizontal) {
        $dgv.ScrollBars = [System.Windows.Forms.ScrollBars]::Horizontal  # 仅横向
    }
    elseif ($needVertical) {
        $dgv.ScrollBars = [System.Windows.Forms.ScrollBars]::Vertical  # 仅纵向
    }
    else {
        $dgv.ScrollBars = [System.Windows.Forms.ScrollBars]::None  # 都不显示
    }
}
