...
# 添加鼠标按下事件 - 显示密码
$script:textNewPassword.Add_MouseDown({
    $this.PasswordChar = $null  # 取消密码掩码，显示明文
})

# 添加鼠标释放事件 - 恢复掩码
$script:textNewPassword.Add_MouseUp({
    $this.PasswordChar = '*'    # 恢复密码掩码
})

# 添加鼠标离开事件 - 确保离开时恢复掩码
$script:textNewPassword.Add_MouseLeave({
    $this.PasswordChar = '*'    # 恢复密码掩码
})
...
# 添加鼠标按下事件 - 显示密码
$script:textConfirmPassword.Add_MouseDown({
    $this.PasswordChar = $null  # 取消密码掩码，显示明文
})

# 添加鼠标释放事件 - 恢复掩码
$script:textConfirmPassword.Add_MouseUp({
    $this.PasswordChar = '*'    # 恢复密码掩码
})

# 添加鼠标离开事件 - 确保离开时恢复掩码
$script:textConfirmPassword.Add_MouseLeave({
    $this.PasswordChar = '*'    # 恢复密码掩码
})
