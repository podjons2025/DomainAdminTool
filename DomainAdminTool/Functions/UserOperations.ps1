...
function LoadUserList {
    ...
    try {
        $script:connectionStatus = "正在加载 OU: $($script:currentOU) 下的用户..."  # 修改状态信息
        UpdateStatusBar
        $script:mainForm.Refresh()
        
        $script:allUsers.Clear()

        # 远程加载用户 - 添加 -SearchBase 参数过滤当前OU
        ...
        $bindingSource.DataSource = $script:allUsers
        $script:userDataGridView.DataSource = $bindingSource
        
        $script:userCountStatus = $script:allUsers.Count
        $script:connectionStatus = "已加载 OU: $($script:currentOU) 下的 $($script:userCountStatus) 个用户"  # 更新状态
        UpdateStatusBar
