...
function LoadGroupList {
    ...
    try {
        $script:connectionStatus = "正在加载 OU: $($script:currentOU) 下的组..."  # 修改状态信息
        UpdateStatusBar
        $script:mainForm.Refresh()
        
        $script:allGroups.Clear()
        
        # 远程执行Get-ADGroup - 添加 -SearchBase 参数过滤当前OU
        $remoteGroups = Invoke-Command -Session $script:remoteSession -ScriptBlock {
            param($searchBase, $allUsersOU)  # 接收当前OU作为参数
			...
			if ($allUsersOU){
				...
				# 使用 -SearchBase 限定只查询指定OU下的组
				Get-ADGroup -Filter * -SearchBase $searchBase `
					-Properties Name, SamAccountName, Description `
					| Select-Object Name, SamAccountName, Description
			}
        } -ArgumentList $script:currentOU, $script:allUsersOU -ErrorAction Stop  # 传递当前OU作为参数
        
        # 填充本地缓存
        $remoteGroups | ForEach-Object { $script:allGroups.Add($_) | Out-Null }
        
        # 使用BindingSource绑定数据源
        $script:groupDataGridView.DataSource = $null
        $groupBindingSource = New-Object System.Windows.Forms.BindingSource
        $groupBindingSource.DataSource = $script:allGroups
        $script:groupDataGridView.DataSource = $groupBindingSource
        
        # 强制刷新DataGridView界面
        $groupDataGridView.Refresh()
        $groupDataGridView.Update()
        
        # 同步应用当前搜索过滤
        FilterGroupList $textGroupSearch.Text
        
        # 更新状态
        $script:groupCountStatus = $script:allGroups.Count
        $script:connectionStatus = "已加载 OU: $($script:currentOU)"
        UpdateStatusBar
